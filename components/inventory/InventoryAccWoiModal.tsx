...
//   (2) 목표+리오더   : "reorder추가시" 모달과 동일 — 리오더 성장률(110%)로 표를 다시 역산한 뒤 리오더 물량 가산
              ...
              {ROW_KEYS.map((key) => {
                ...
                return (
                  <tr key={`woi-${key}`} className={rowCls(key)}>
                    ...
                    <td className={tdNum}>{r ? formatWoi(r.woi) : ''}</td>
                    <td className={tdNum}>{isTotal && plWoi != null ? formatWoi(plWoi) : ''}</td>
                  </tr>
                );
              })}
            </tbody>
          ...
          <table className="w-full table-fixed border-collapse">
            ...
            <tbody>
              {ROW_KEYS.map((key) => {
                ...
                return (
                  <tr key={`si-${key}`} className={rowCls(key)}>
                    ...
                    <td className={tdNum}>{r ? fmtK(r.sellInTotal + ro) : ''}</td>
                    <td className={tdNum}>{isTotal ? fmtK(plAccSellInK) : ''}</td>
                  </tr>
                );
              })}
              <tr className={footRowCls}>
                ...
                <td className={`${tdNum} ${reorderBg}`} />
                <td className={tdNum}>{fmtK(re['ACC합계']?.closing)}</td>
                <td className={tdNum}>{fmtK(plClosing)}</td>
              </tr>
              <tr className={footRowCls}>
                <td className={tdLabel}>리테일</td>
                <td className={tdNum}>{fmtK(acc?.sellOutTotal)}</td>
                <td className={`${tdNum} ${reorderBg}`} />
                <td className={tdNum}>{fmtK(re['ACC합계']?.sellOutTotal)}</td>
                <td className={tdNum}>{fmtK(acc?.sellOutTotal)}</td>
              </tr>
            </tbody>
          </table>
