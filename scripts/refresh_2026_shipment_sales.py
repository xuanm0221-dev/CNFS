"""
2026년 출고매출(본사→대리상) 전처리 스크립트
==========================================
로컬 Next.js 서버(localhost:3000)에 API 요청 → Snowflake 조회 →
public/data/inventory/2026/shipment-sales-{brand}.json 저장

사용법:
  python scripts/refresh_2026_shipment_sales.py --baseMonth 2          # 1~2월 실적
  python scripts/refresh_2026_shipment_sales.py --baseMonth 3          # 1~3월 실적
  python scripts/refresh_2026_shipment_sales.py --baseMonth 2 --brand MLB  # MLB만
...
def fetch_brand(brand: str, closed_through: str) -> bool:
    url = (
...
def main():
    parser = argparse.ArgumentParser(description="2026년 출고매출 전처리")
    parser.add_argument(
        "--baseMonth",
        type=int,
        required=True,
    ...
    print("=" * 55)
    print(f"  2026년 출고매출 전처리  (실적: 1~{args.baseMonth}월)")
    print(f"  closedThrough={closed_through}")
    print("=" * 55)
    ...
    success = 0
    for brand in brands:
        ok = fetch_brand(brand, closed_through)
        if ok:
            success += 1
        time.sleep(0.5)
